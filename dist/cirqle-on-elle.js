...
(function(){
  ...
  try{
    GATrack.saveCustomGADomainPair({domainUrl:'www.elle.nl', ga:'UA-61193809-1'});
    GATrack.trackView();
  }catch(e){}
        ...
        var observer = new MutationObserver(function(mutations) {
          ...
          for(var i=0; i < mutations.length; i++) {
            ...
            for(var j=0; j < addedNodes.length; j++){
              ...
              // image placeholder reused by ohter image urls
              if(target.querySelectorAll){
                var img = target.querySelectorAll('img');
                if(img.length > 0){
                  var btns = buttonSingleton.getAllButtons();

                  btns.buttonCreated = function(url){
                    for(var i=0; i<this.length; i++){
                      if(this[i].btn.imgSrc == url) return this[i];
                    }
                    return null;
                  };
                  buttonSingleton.getTaggedImg().then(function(imgUrls){
                      for(var i=0; i<img.length; i++){
                        if(img[i].src && imgUrls.indexOf(img[i].src) > -1){
                          // console.log(img[i]);
                          var btnCreated = btns.buttonCreated(img[i].src);
                          // src is being replace, button insert here, usually happens on slider
                          if(!btnCreated){
                            // create button
                            dataset(img[i]).del("postId");
                            dataset(img[i]).del("cqUuid");
                            cqjq(".cirqle-outer-button").hide();
                            createButtonByUrl(document, img[i].src);
                          }
                          else{
                            var uuid = dataset(btnCreated.btn, 'uuid');
                            // dataset(img[i], "cqUuid", uuid);
                            // cqjq(".cirqle-outer-button").hide();
                            // cqjq(btnCreated).show();
                            repositionButton();
                            // button created
                          }
  ...
  function repositionButton(){
    ...
    if(buttons.length == 0){return;}
      ...
      for(var i=0; i < buttons.length; i++){
        ...
        var btndata = dataset(btnObj.btn);
        var imgdata = dataset(btnObj.img);
        if(btnObj.btn.style.position == "relative"){
          continue; // exclude button posistioned by relative
        }
        else if(isHidden(btnObj.img) || btndata.get("uuid") != imgdata.get("cqUuid") || findIntersectors(document.body, btnObj.img) > 0){
          cqjq(btnObj.btn).hide();
          continue;
        }
        else{
          cqjq(btnObj.btn).show();
          positionButtonAbsolute(btnObj.img, btnObj.btn);
          if(findIntersectors(document.body, btnObj.img) > 0)
            cqjq(btnObj.btn).hide();
  ...
  function getTaggedImgFromApi(){
    return cqjq.getJSON(cirqle_getpost_by_url+"&callback=?");
  }
  ...
  function findPostImage(scope, post_img_identifier){
    ...
    buttonSingleton.getTaggedImg().then(function(imgUrls){
      for(var i = imgUrls.length-1; i >=0; i--){
        createButtonByUrl(scope, imgUrls[i]);
      }
    });
  ...
  function createButtonByUrl(scope, url){
    var selector = "img[src*='"+getElleImageName(url)+"']";
      ...
      if(imgElm && !dataset(imgElm, "cqUuid")){
        (function(imgElm, i){
          if(imgElm.height == 0 || imgElm.width == 0){
            cqjq("<img/>")
            .load(function() {
              getPostImageInfo(url).then(function(data){
                if(data.length > 0){
  ...
  function getPostImageInfo(imgurl){
    var url = apiDomain + "/api/1/posts/products?url="+encodeURIComponent(imgurl)+"&blogId="+blog_id;
    return cqjq.getJSON(url+'&callback=?');
  }
  /*imgIdentifyMethods.page = true*/
  ...
  function embeddShopButton(imgNode, imgObj, ifrmScope){
    ...
    // remove button with image smaller dimension than the button
    if(button.width < imgNode.width){
        try {
...
function highestZIndex(defaultIndex, btn){
  ...
  function getElleImageName(imageURL){
      // return imageURL.substring(0, imageURL.lastIndexOf(".")).replace(/^.*\/\/[^\/]+/, '');
...
},{"./modules/dataset":2,"./modules/track":3}],2:[function(require,module,exports){
...
//# sourceMappingURL=map/cirqle-on-elle.js.map