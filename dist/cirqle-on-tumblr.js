...
(function(){
  ...
  var css_url = "//cdn.cirqle.nl/button1/cirqle-style-general.css";
  ...
  function findPostImage(scope, post_img_identifier){
    ...
    buttonSingleton.getTaggedImg().then(function(imgUrls){
      for(var i = imgUrls.length-1; i >=0; i--){
        var selector = "img[src*='"+getTumblrImageId(imgUrls[i])+"']";
  ...
  function positionButtonAbsolute(imgNode, btnNode){
    ...
    if(isNaN(current)) btnNode.style.zIndex = highestZIndex(1, btnNode)+1;
  ...
  function showWindow(img, timer, trackTraits){
    ...
    iframeDiv.style.zIndex = highestZIndex(1, iframeDiv)+2; // button is +1
...
//# sourceMappingURL=map/cirqle-on-tumblr.js.map